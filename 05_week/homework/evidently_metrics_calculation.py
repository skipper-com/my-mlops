...
def prep_db():
    with psycopg.connect(
        ...
        with psycopg.connect(
            "host=localhost port=6432 dbname=homework user=postgres password=qwerty123"
        ) as conn:
...
def calculate_metrics_postgresql(curr, i):
    ...
    curr.execute(
        "insert into homework(timestamp, prediction_drift, num_drifted_columns, share_missing_values, quantile_metric) values (%s, %s, %s, %s, %s)",
        (
